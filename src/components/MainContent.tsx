import {useTodos, useSharedTodos, useFilterStore, useSearchStore, useUserStore } from "../store";
...
const MainContent = () => {
    ...
    if (!user || useTodos.getState().getTodosByUserId(user.id)?.length === 0) {
        return <div className="flex justify-center items-center h-full w-full md:w-4/5 mt-12">
            <div
                className="bg-gray-300 w-11/12 sm:w-[30rem] h-[16rem] rounded-2xl overflow-hidden flex flex-col items-center gap-4 p-4">
            ...
            <div
                className="w-11/12 backdrop-blur-xs fixed rounded-2xl flex justify-center items-center h-[16rem] sm:w-[30rem]">
                {user ? <AddTodoButton/> :
                    <LoginButton icon={true} textQuery="max-sm:text-sm" mediaQuery="max-md:w-26 rounded-lg"/>}
            </div>
        </div>;
    }
    ...
    switch (filterType) {
        ...
        case "year":
            ...
            break;
        case "shared todos":
            filteredTodos = useSharedTodos.getState().getSharedTodosByUserId(user.id) || [];
            break;
        case "all":
        default:
